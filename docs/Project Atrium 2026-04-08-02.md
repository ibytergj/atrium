...
│   └── som-inspector/                  # SOM Inspector (Sessions 19, 22–25)
│       ├── index.html
│       └── src/
│           ├── app.js
│           ├── TreeView.js
...
│   │   ├── space-ext.atrium.json  # World manifest for space-ext
...
pulling latest. Server count grew from 32 to ~81 with external
reference resolution tests (Session 25).
...
`client._worldBaseUrl`

**Methods:**
```javascript
client.connect(wsUrl, { avatar: descriptor })
client.disconnect()
client.loadWorld(url)               // fetch glTF from URL, init SOM
...
**`_worldBaseUrl` (Session 24):** Set by `loadWorld(url)` as an absolute
URL: `new URL(url, window.location.href).href`. This ensures relative
paths in `extras.atrium.source` resolve correctly regardless of whether
the original URL was relative or absolute. Set to `null` by
`loadWorldFromData()` (no URL context available).
...
## SOM Inspector (`tools/som-inspector/`) — Sessions 19, 22, 23
...
**Status (Session 23):** Consumed by `apps/client` and the SOM Inspector.
Loading an `.atrium.json` URL via the URL bar resolves the glTF path
relative to the config file URL, loads the world, and populates the
server/Connect field. Dropping an `.atrium.json` file also works — for
absolute glTF URLs it loads the world; for relative paths (local drop) it
populates the server field and shows guidance to drop the `.gltf` directly.
Does not auto-connect.

The server takes `WORLD_PATH` env var pointing directly to the `.gltf`.
Future: server could also read `.atrium.json` for its config.
...
- **Client:** `_worldBaseUrl` is set by `loadWorld(url)`, null'd by
  `loadWorldFromData` — external references in dropped files silently
  skip resolution
- **Server:** `worldBaseUrl` derived from `WORLD_PATH` via
  `pathToFileURL(resolve(gltfPath))`. Uses `fs.readFile` for `file://`
  URLs, `fetch` for `http(s)://`
...
**`_worldBaseUrl` resolution:** `loadWorld(url)` resolves the URL to
absolute via `new URL(url, window.location.href).href` before storing.
Without this, relative paths like `../../tests/fixtures/space-ext.gltf`
cause `new URL(source, base)` to fail when resolving external reference
paths.
...
- **External refs via `som-dump` (connect-only) have no base URL** —
  when a client connects to a server without first loading the world
  via the URL bar, `_worldBaseUrl` is null and external references
  cannot resolve. The Load-then-Connect flow works. Attempted fix in
  Session 25 (send `worldBaseUrl` in server `hello`) was reverted
  because the server derives a `file://` URL from `WORLD_PATH`, which
  browsers cannot fetch. The correct fix is `.atrium.json` consumption
  by the server, which provides the HTTP URL where the world assets
  are served. (Session 24, attempted Session 25)
...
### Architecture
- **`.atrium.json` consumption by server** — server reads the config
  file for `WORLD_PATH`, port, and crucially the HTTP base URL for
  the world assets. The base URL is sent to clients in the `hello`
  message so connect-only clients can resolve external references.
  This replaces the reverted `pathToFileURL` approach (which produced
  `file://` URLs browsers can't fetch). (Session 24 gap, Session 25
  revised)
