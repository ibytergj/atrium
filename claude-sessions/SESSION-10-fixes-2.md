...
The UI is two columns. The **left column** has SEND MESSAGE on top and the
SOM tree below it. The **right column** has the scene view on top and the
message log below it. The SOM tree is collapsible (collapsed by default).
...
- [ ] SOM source copy is in sync:
      `cp packages/som/src/SOMDocument.js tests/client/som/SOMDocument.js`
- [ ] SOM tree view panel renders in left column, below the SEND MESSAGE form; scene view and message log are in the right column
